...
function sendTempMessageCard(message, id) {
  return `
<div class="ms-panel-body ms-scrollable" data-id="${id}">
  <p>
      ${message}
      <sub>
...
function errorMessageCard(id) {
  ...
  messagesContainer
    ...
    .remove();
  messagesContainer
    .find(".ms-panel-body[data-id=" + id + "] p")
  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        $(".ms-panel-header .user-name").text(data.fetch.name);
        // Star status
        data.favorite > 0
      ...
      success: (data) => {
        if (data.error > 0) {
          ...
          const tempMsgCardElement = messagesContainer.find(
            `.ms-panel-body[data-id=${data.tempID}]`
          );
      ...
      success: (data) => {
        ...
        if (messagesPage == 1) {
          ...
          const lastMsg = messagesElement.find(
            messagesElement.find(".ms-panel-body")[0]
          );
          const curOffset =
...
clientListenChannel.bind("client-messageDelete", function (data) {
  $("body").find(`.ms-panel-body[data-id=${data.id}]`).remove();
});
...
function deleteMessage(id) {
  $.ajax({
    ...
    success: (data) => {
      $(".messages").find(`.ms-panel-body[data-id=${id}]`).remove();
      if (!data.deleted)
